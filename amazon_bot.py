...
def screen_shot():
    ...
    image = ImageGrab.grab(bbox=(1118, 68, 1240, 82))
...
class AmazonBot:
    ...
    def log_on(self):
        ...
        sleep(4)  # Wait a bit for the message to appear on your computer screen
    ...
    def purchase(self):
        items = ["ramen x2 spicy", "bubblegum", "guitar capo"]  # Replace this with whatever you want to buy
        for i in range(0, len(items)):  # The for loop that does your shopping
            ...
            products = bot.driver.find_elements_by_class_name('sg-col-inner')
            ...
            try:
                ...
                bot.driver.find_element_by_class_name('a-icon-radio-inactive').click()
                sleep(1)
                # Add to cart
                bot.driver.find_element_by_id('add-to-cart-button').click()
                ...
                bot.driver.find_element_by_id('add-to-cart-button').click()
            sleep(1)
            bot.driver.get('https://amazon.com')  # Go back to the main page to buy more stuff
